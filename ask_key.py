...
def check_key(key: str) -> tuple[bool, str]:
    ...
    return True, f"key works — {len(names)} models visible, {len(flash)} flash"
...
def main() -> int:
    ...
    status.grid(column=0, row=3, columnspan=3, sticky="w", pady=(10, 0))
    ...
    def on_save() -> None:
        ...
        write_env(vals)
        result["saved"] = True
        say(f"saved to {ENV.name}", "#070")
        root.after(700, root.destroy)

    btns = ttk.Frame(frm)
    btns.grid(column=0, row=4, columnspan=3, sticky="e", pady=(14, 0))
    ttk.Button(btns, text="Test", command=on_test).grid(column=0, row=0, padx=(0, 8))
    ttk.Button(btns, text="Save", command=on_save).grid(column=1, row=0, padx=(0, 8))
    ttk.Button(btns, text="Cancel", command=root.destroy).grid(column=2, row=0)
